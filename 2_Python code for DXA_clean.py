...
if __name__ == "__main__":
    ...
    Output_CSV_path=r"\\cdATSHFS.intel.com\CDATAnalysis$\MAOATM\Longchao\DIA_clean_PBI_report\DXAclean_logrecord.csv"
    import os
    ...
    for ( roots, dirs, files ) in os.walk (File_path):
        for file in files:
            if file.endswith('.xml'):
                continue
            if os.path.basename(roots)[0:3] == 'TGB':
    ...
    for i,file in enumerate(all_logfiles):
        with open(file,'r') as f:
            ...
            for line in lines: # Get aligned door open and close log with key word and date.
                if re.match('tCooldown \[\d{2}:\d{2}:\d{2},\d{3}\]: ihscooling.cpp:1213',line):#TGB cooldown
                    # print(line)
                    ...
                    DSA_dooropen_close_loglist.append([all_toolentity[i],LogDate,line[line.find('Cooldown done'):].strip()])
                # if re.match('\d{2}:\d{2}:\d{2}.\d{4} AclPlusDrv.cpp\(\d{5}\) Door',line):#ASF door open and close
                #     # print(line)
                #     LogDate=datetime.strftime(datetime.fromtimestamp(os.stat(file).st_birthtime).replace(hour=int(line[line.find('[')+1:line.find('[')+3]),minute=int(line[line.find('[')+4:line.find('[')+6]),second=int(line[line.find('[')+7:line.find('[')+9]),microsecond=0),'%Y-%m-%d %H:%M:%S')
